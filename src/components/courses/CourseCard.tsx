        ...
        <div className="p-5 flex-1">
          <div className="flex justify-between items-start mb-3">
            <div>
              <div className="space-y-1">
                <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t('card.course')}</span>
                ...
                {course.shift && (
                  <p className={`text-sm font-bold mt-1.5 ${course.shift === 'Mañana' ? 'text-amber-600 dark:text-amber-400'
                    ...
                    {t('card.shift')}: {
                      course.shift === 'Mañana' ? t('shifts.morning') :
                      course.shift === 'Tarde' ? t('shifts.afternoon') :
                      course.shift === 'Noche' ? t('shifts.night') : course.shift
                    }
                  </p>
                )}
              </div>
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-bold px-3 py-1 rounded-lg shrink-0 ml-3 border border-indigo-100 dark:border-indigo-800">
              {t('card.students', { count: course.meta.studentCount })}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2.5">
            <div className="flex items-center text-sm font-medium text-gray-600 dark:text-gray-300">
              ...
              <span>{t('card.startDate')}: <span className="text-gray-900 dark:text-gray-100">{startDate}</span></span>
            </div>

            <div className="flex items-center text-sm font-medium text-gray-600 dark:text-gray-300">
              <BarChart3 className="w-5 h-5 mr-3 text-indigo-500/70" />
              <span>{t('card.attendance')}: <span className="text-gray-900 dark:text-gray-100">{attendancePercent}% {t('card.avg')}</span></span>
            </div>
          </div>
        </div>
