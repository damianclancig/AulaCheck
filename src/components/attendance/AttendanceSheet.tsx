...
interface StudentAttendanceCardProps {
  ...
  suspensions?: Record<string, { reason: string; note?: string }>;
  onContextMenu: (e: React.MouseEvent | React.TouchEvent, studentId: string, date: string, status?: AttendanceStatus) => void;
              ...
              return (
                ...
                >
                  <div className={`text-xs font-medium text-center flex flex-col items-center ${isSuspended
                    ? 'text-blue-600 dark:text-blue-400' : isFuture
                    ? 'text-gray-400 dark:text-gray-500 italic' : 'text-gray-600 dark:text-gray-400'
                  }`}>
                    {(() => {
...
export function AttendanceSheet({ students, dates, records, suspensions, onUpdate }: AttendanceSheetProps) {
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent | React.TouchEvent, studentId: string, date: string, currentStatus?: AttendanceStatus) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {contextMenu.visible && (
        ...
        >
          <button onClick={() => handleStatusChange('present')} disabled={updating} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-3 disabled:opacity-50 text-gray-700 dark:text-gray-300 transition-colors">
            <Check className="w-4 h-4 text-green-600 dark:text-green-400" />
            <span className="font-medium">{t('legends.present')}</span>
          </button>
          <button onClick={() => handleStatusChange('absent')} disabled={updating} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-3 disabled:opacity-50 text-gray-700 dark:text-gray-300 transition-colors">
            <XCircle className="w-4 h-4 text-red-600 dark:text-red-400" />
            <span className="font-medium">{t('legends.absent')}</span>
          </button>
          <button onClick={() => handleStatusChange('late')} disabled={updating} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-3 disabled:opacity-50 text-gray-700 dark:text-gray-300 transition-colors">
          ...
          {contextMenu.currentStatus && (
            <>
              ...
              <button onClick={() => handleStatusChange(null)} disabled={updating} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-3 disabled:opacity-50 text-gray-600 dark:text-gray-400 transition-colors">
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
            <tr>
              ...
              {dates.map((date) => {
                ...
                const isFuture = isFutureDate(date);
                return (
                  <th key={date} className={`px-4 py-3 text-center text-xs font-medium uppercase tracking-wider whitespace-nowrap group relative ${suspension ? 'bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400' : isFuture ? 'text-gray-400 dark:text-gray-500 bg-gray-50/50 dark:bg-gray-800/50' : 'text-gray-500 dark:text-gray-400'}`}>
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {displayedStudents.map((student) => {
              ...
              return (
                <tr key={studentId} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  {dates.map((date) => {
                    const status = studentRecords[date];
                    const isDisabled = !!suspensions?.[date] || isFutureDate(date) || (student.enrollmentStatus === 'inactive' && !status);
                    return (
                      <td key={date} className={`px-4 py-4 text-center ${isDisabled ? 'bg-gray-50/50 dark:bg-gray-800/50 cursor-default' : 'cursor-context-menu'}`} onContextMenu={(e) => { if (!isDisabled) handleContextMenu(e, studentId, date, status); else e.preventDefault(); }}>
                        <div className="flex justify-center">
                          {isDisabled && !status ? (
                            <div className={`rounded-full ${!!suspensions?.[date] ? 'w-2 h-2 bg-gray-300' : isFutureDate(date) ? 'w-1 h-1 bg-gray-200' : 'w-4 h-0.5 bg-gray-300'}`} />
                          ) : <AttendanceIcon status={status} size="md" />}
                        </div>
                      </td>
                    );
      ...
      <div className="md:hidden space-y-4">
        {displayedStudents.map((student) => (
          <StudentAttendanceCard
            ...
            scrollRef={(el) => { if (el) scrollRefs.current.set(student._id.toString(), el); }}
