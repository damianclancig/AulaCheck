...
async function createIndexes() {
  try {
    ...
    await db
      ...
      .createIndex({ courseId: 1, studentId: 1, date: 1 }, { unique: true })
    ...
    // Índices para joinRequests
    await db.collection('joinRequests').createIndex({ courseId: 1, status: 1, createdAt: -1 })
    await db.collection('joinRequests').createIndex(
      { courseId: 1, deviceTokenHash: 1, status: 1 },
      {
        unique: true,
        partialFilterExpression: {
          status: 'pending',
          deviceTokenHash: { $exists: true },
        },
      },
    )
    console.log('✓ Created indexes on joinRequests')
